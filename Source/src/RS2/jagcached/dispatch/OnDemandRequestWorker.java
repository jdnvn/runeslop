...
import RS2.jagcached.net.jaggrab.OnDemandResponse;
...
public final class OnDemandRequestWorker extends RequestWorker<OnDemandRequest> {
	...
	protected void service(IndexedFileSystem fs, Channel channel,
		...
		for (int chunk = 0; buf.remaining() > 0; chunk++) {
			...
			ChannelBuffer chunkData = ChannelBuffers.wrappedBuffer(tmp, 0,
					chunkSize);

			OnDemandResponse response = new OnDemandResponse(desc, length,
					chunk, chunkData);
