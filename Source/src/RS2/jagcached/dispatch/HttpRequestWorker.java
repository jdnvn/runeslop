...
public final class HttpRequestWorker extends RequestWorker<HttpRequest> {
	...
	@Override
	protected ChannelRequest<HttpRequest> nextRequest()
	...
	protected void service(IndexedFileSystem fs, Channel channel,
			...
			if (target.toASCIIString().startsWith(base.toASCIIString())) {
				if (f.exists()) {
					...
					if (f.isDirectory()) {
						...
						wrappedBuf = createErrorPage(status,
								"Directory listings cannot be viewed.");
						mimeType = "text/html";
					} else {
		...
		resp.setHeader("Content-type",
				mimeType + ", charset=" + CHARACTER_SET.name());
	 ...
	 */
	private ChannelBuffer createErrorPage(HttpResponseStatus status,
			String description) {
		...
		return ChannelBuffers.copiedBuffer(bldr.toString(),
